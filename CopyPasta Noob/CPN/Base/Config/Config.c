...
VOID Config_load(UINT id)
{
    ...
    if (json) {
        ...
        {
            cJSON* styleJson = cJSON_GetObjectItem(json, "Style");

            cJSON* menuStyle = cJSON_GetObjectItem(styleJson, "Menu style");
            if (cJSON_IsNumber(menuStyle)) config.style.menuStyle = menuStyle->valueint;

            cJSON* menuColor = cJSON_GetObjectItem(styleJson, "Menu color");
            if (cJSON_IsNumber(menuColor)) {
                config.style.menuColors = menuColor->valueint; 
            }
        }
    }
...
VOID Config_save(UINT id)
{
    ...
    {
        cJSON* styleJson = cJSON_CreateObject();

        cJSON_AddNumberToObject(styleJson, "Menu style", config.style.menuStyle);
        cJSON_AddNumberToObject(styleJson, "Menu colors", config.style.menuColors);

        cJSON_AddItemToObject(json, "Style", styleJson);
    }
