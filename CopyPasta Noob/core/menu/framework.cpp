...
void menu_framework::group_box(std::int32_t x, std::int32_t y, std::int32_t w, std::int32_t h, unsigned long font, const std::string string, bool show_label) {
	...
	//groupbox background
	render::draw_filled_rect(x, y, w, h, menu_params::groupbox_background);

	//groupbox outline
	render::draw_rect(x, y, w, h, menu_params::groupbox_outline);

	//groupbox label
	if (show_label)
		render::draw_text_string(x + 2, y - 12, font, string, false, menu_params::groupbox_label);
}
...
void menu_framework::tab(std::int32_t x, std::int32_t y, std::int32_t w, std::int32_t h, unsigned long font, const std::string string, std::int32_t& tab, std::int32_t count, bool show_outline) {
	...
	//tab background
	if (show_outline)
		render::draw_rect(x, y, w, h, tab == count ? menu_params::tab_background1 : menu_params::tab_background2);

	//tab label
	render::draw_text_string(x - render::get_text_size(font, string).x / 2 + 50, y + h / 2 - 8, font, string, false, show_outline ? menu_params::tab_outline : tab == count ? menu_params::tab_selected : menu_params::tab_nonselected);
}
...
void menu_framework::check_box(std::int32_t x, std::int32_t y, std::int32_t position, unsigned long font, const std::string string, bool& value) {
	...
	//checkbox background
	render::draw_filled_rect(position, y, w, h, value ? menu_params::checkbox_selected : menu_params::checkbox_background);

	//checkbox label
	render::draw_text_string(x + 2, y - 1, font, string, false, menu_params::checkbox_label);
}
...
void menu_framework::slider(std::int32_t x, std::int32_t y, std::int32_t offset, std::int32_t lenght, unsigned long font, const std::string string, float& value, float min_value, float max_value) {
	...
	//slider background
	render::draw_filled_rect(offset, yi, lenght, 6, menu_params::slider_background);
	render::draw_filled_rect(offset, yi, value * (float(lenght) / float(max_value)), 6, menu_params::slider_selected);

	//slider label
	render::draw_text_string(x + 2, y - 1, font, (std::stringstream{ } << string << ": " << std::setprecision(3) << value).str(), false, menu_params::slider_label);
}
